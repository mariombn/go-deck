...
- **Manter a tela ligada (celular).** Botão no cabeçalho do `MobileApp` (`useKeepAwake`). **Pegadinha:** a Screen Wake Lock API nativa (`navigator.wakeLock`) só existe em **contexto seguro** (HTTPS/localhost) — e o celular acessa por `http://IP` da LAN, que **não** é seguro. Por isso usamos **`NoSleep.js`** (vídeo invisível em loop; recorre à Wake Lock API sozinho se um dia houver HTTPS). `enable()` exige um **gesto do usuário** (o clique no botão atende) — por isso o estado **não** é auto-restaurado entre reloads; reativa ao voltar de segundo plano via `visibilitychange`.
...
- **Auth por token de pareamento (sem HTTPS).** O upgrade do WS valida **`Origin`** (`sameOrigin`) **e** um **token de pareamento** (`?t=` na query). O token (`Server.Token`, 16 bytes hex) é gerado no `config.normalize`, **persistido** em `config.json` (sobrevive a reinícios → o celular reconecta sozinho) e **nunca enviado ao frontend** (`config.Store.clone()` o zera). Ele viaja **só na URL do QR** (`server.Network()` anexa `?t=`); o celular o lê de `window.location.search` e o reenvia no handshake. Comparação em tempo constante (`crypto/subtle`). `Replace` preserva o token quando a config que volta do desktop vem sem ele. **O celular continua enviando só id de botão**, nunca um `path`. Mitigações que faltam (ainda não é hardening total): **sem HTTPS** ⇒ o token trafega na LAN em texto puro (sniffável em rede hostil) e a senha do OBS segue em texto puro no `config.json`; **sem rotação** de token pela UI.
...
- `main.tsx` tem um overlay que exibe erros de runtime na tela — **só em DEV** (`import.meta.env.DEV`, útil debugando no celular sob `wails dev`, sem devtools). Em produção fica desligado (apenas `console.error`), para não expor stack traces ao usuário final.
